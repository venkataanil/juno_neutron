...
from neutron.plugins.ml2.drivers.cisco.ucsm import config as config
...
class CiscoUcsmDriver(object):
    ...
    def check_vnic_type_and_vendor_info(self, vnic_type, profile):
        ...
        if vnic_type not in self.supported_sriov_vnic_types:
            LOG.info(_('Non SR-IOV vnic_type: %s.'), vnic_type)
            return False
    ...
    def _create_vlanprofile(self, vlan_id):
        ...
        if not handle:
            LOG.error(_('UCS Manager network driver failed to connect '
    ...
    def create_portprofile(self, profile_name, vlan_id, vnic_type):
        ...
        if not self._create_vlanprofile(vlan_id):
            LOG.error(_('UCS Manager network driver failed to create '
    ...
    def update_serviceprofile(self, host_id, vlan_id):
        ...
        """
        service_profile = self.ucsm_host_dict.get(host_id)
        if service_profile:
            LOG.debug("UCS Manager network driver Service Profile : %s",
                service_profile)
        else:
            LOG.info(_('UCS Manager network driver does not support Host_id '
                       '%s'), str(host_id))
            return
        ...
        if not self._update_service_profile(service_profile, vlan_id):
            LOG.error(_('UCS Manager network driver failed to update '
    ...
    def _delete_port_profile(self, port_profile):
        ...
        try:
            ...
            if not p_profile:
                LOG.warning(_('UCS Manager network driver did not find Port '
